...
var filterValue = ''
var pageLoadCount = 1
var CablesKmlUrl = "./kmls/cables-map.kml"
if (PATHHS === undefined)
    var PATHHS = ''
...
KmlMapParser = function (config) {
    ...
    this.sidebarId = ['map-sidebar', 'modal-popup'];
    ...
    this.defaultImageSize = (config.imageSize !== undefined) ? this.config.imageSize : {
        width: 20,
        height: 20
    };
    this.defaultHotspot = (config.imageHotspot !== undefined) ? this.config.imageHotspot : {
        //x: 16,
        //y: 32
        x: 10,
        y: 16
    ...
    this.defaultZoomLevel = (config.zoomLevel !== undefined) ? this.config.zoomLevel : 4;
...
KmlMapParser.prototype = {
    ...
    setVisibility: function (isVisible) {
        ...
        if (this.showSidebar && !!this.sidebarId[0]) {
            var side = document.getElementById(this.sidebarId[0]);
            var modal_popup = document.getElementById(this.sidebarId[1]);
            if (side !== null) {
                this.setVisibilitybyNode(side, isVisible)
                this.setVisibilitybyNode(modal_popup, isVisible)
            }
        }
    },
    ...
    clearMap: function () {
        if (this.docSet && this.docSet.docs) {
            ...
            if (this.showSidebar && !!this.sidebarId[0]) {
                side = document.getElementById(this.sidebarId[0]);
                modal_popup = document.getElementById(this.sidebarId[1]);
                if (side !== null) {
                    // side.innerHTML = ""
    ...
    filterSidebar: function (text) {
        ...
        var t = text.toLowerCase().trim(),
            isVisible, side;
        let filterClass = 'unknown';
        let filterStatus = true;
        (document.getElementById('cable').checked) ?
        (document.getElementById('popSite').checked) ?
        filterStatus = true: filterClass = 'popSite':
            (document.getElementById('popSite').checked) ?
            filterClass = 'cable' : filterStatus = false;
        if (this.showSidebar && !!this.sidebarId[0]) {
            side = document.getElementById(this.sidebarId[0]);
            if (side !== null) {
                ...
                if (nodes !== null && nodes.length > 0) {
                    for (let i = 0; i < nodes.length; i++) {
                        node = nodes[i];
                        if (node.getAttribute("data-tag") === filterClass && filterStatus) {
                            this.setVisibilitybyNode(node, false)
                        } else if (filterStatus === false) {
                            this.setVisibilitybyNode(node, false)
                        } else {
                            this.setVisibilitybyNode(node, true)
    ...
    convertKmlStyle: function (style) {
        ...
        if (style && style.href !== undefined && style.href !== "") {
            ...
            if (this.showImageShadow) {
                ...
                if (regEx.test(href)) {
                    style.shadow = new google.maps.MarkerImage("http://maps.google.com/mapfiles/ms/micons/msmarker.shadow.png", shadowSize, zeroPoint, anchorPoint)
                } else {
                    if (href.indexOf("-pushpin.png") > -1) {
                        style.shadow = new google.maps.MarkerImage("http://maps.google.com/mapfiles/ms/micons/pushpin_shadow.png", shadowSize, zeroPoint, anchorPoint)
                    }
                }
            }
        }
    ...
    render: function (responseXML, doc, scope) {
        if (!responseXML) {} else {
            if (!doc) {
                ...
                doc.overlays = [];
                doc.isVisible = true;
                var styleId, pnNode, styleNode, styleNodeId, nodes = responseXML.getElementsByTagName("Style"),
                ...
                var folderNodes = responseXML.getElementsByTagName("Folder"),
                    placemarkNodes, parentFolder = doc;
                ...
                if (scope.showSidebar && pageLoadCount <=2) {
    ...
    findFolders: function (doc, folderNode, parentFolder) {
        var placemarkNodes, groundNodes, parentName = parentFolder.name,
    ...
    createPlacemark: function (doc, placemarkNode, folder) {
        var style, docStyles = doc.styles,
            placemark = {
                folderId: folder.id,
    ...
    createMapObjects: function (doc) {
        if (!!google.maps) {
            var pm, i, j, placemark, shapes, shape, latlng, coords, coord, path, bounds, poly, found = false,
            ...
            google.maps.event.addListener(marker, "click", function () {
                ...
                for (var x = 0; x < markers.length; x++) {
                    if (markers[x].id === marker.id) {
                        // markers[x].icon.url = 'http://omaps.neomeric.us/images/circle-blk-sm.png'
                    } else {
                        // markers[x].icon.url = 'http://omaps.neomeric.us/images/circle-o-sm.png'
                    }
                }
            })
        }
    ...
    processShape: function (geometry, placemark, node) {
        switch (geometry) {
            case "MultiGeometry":
                ...
                break;
            case "LinearRing":
                placemark.shape.push({
                    polygon: this.getCoords(node, "LinearRing")
                });
                break;
            case "Polygon":
                placemark.shape.push({
            ...
            case "Point":
                placemark.shape.push({
                    point: this.getCoords(node, "Point")[0]
                });
                break;
            case "Circle":
                placemark.shape.push({
                    circle: this.getCoords(node, "Point")[0]
                });
                break;
            case "LineString":
                placemark.shape.push({
                    linestring: this.getCoords(node, "LineString")
                });
                break;
            default:
                break
    ...
    getCoords: function (node, tag) {
        var coordListA = [],
            i, j, coords, path, pathLength, coordList, k, coordNodes = node.getElementsByTagName("coordinates");
        if (!coordNodes) {
            return [{
                coordinates: []
            }]
    ...
    findOverlays: function (doc, groundNodes, folder, parentFolder) {
        ...
        for (i = 0; i < groundNodes.length; i++) {
            node = groundNodes[i];
            groundOverlay = {
                name: this.getNodeValue(node.getElementsByTagName("name")[0]),
                description: this.getNodeValue(node.getElementsByTagName("description")[0]),
                icon: {
                    href: this.getNodeValue(node.getElementsByTagName("href")[0])
                },
                latLonBox: {
                    north: parseFloat(this.getNodeValue(node.getElementsByTagName("north")[0])),
            ...
            var box = groundOverlay.latLonBox,
                bounds = new google.maps.LatLngBounds(new google.maps.LatLng(box.south, box.west), new google.maps.LatLng(box.north, box.east));
    ...
    createOverlay: function (doc, groundOverlay, folder) {
        ...
        var overlay = new KmlOverlayView({
            map: this.map,
            href: groundOverlay.icon.href,
    ...
    createSidebar: function (doc) {
        if (!!this.sidebarId[0]) {
            var childFolders = doc.folders,
                i, placemarks, pm, folder, folderName, side = document.getElementById(this.sidebarId[0]),
                modal_popup = document.getElementById(this.sidebarId[1]),
                id, contents = "",
                innerContents = "";
            folderName = doc.name;
            if (side !== null) {
                ...
                else if(doc.url === CablesKmlUrl && innerContents !== "" ){
                     contents += '<li class="docroot-cbl" id="doc-' + doc.id + '" style="display:block"><ul>' + innerContents + "</ul></li>"
                }
                ...
                if (root !== null) {
                    ...
                    root.appendChild(c)
                    if(doc.url === CablesKmlUrl) {
                        modal_popup.innerHTML = '<ul id="root">' + contents + "</ul>";  
                        PATHHS = contents;
                    } else {
                        modal_popup.innerHTML = PATHHS;  
                    }
                } else {
                    ...
                    if (side.addEventListener) {
                        ...
                        modal_popup.addEventListener("click", clickSidebar, false)
                    } else {
                        if (side.attachEvent) {
                            side.attachEvent("onclick", clickSidebar)
                            modal_popup.addEventListener("click", clickSidebar)
                        }
                    }
                }
            }
    ...
    addSidebarFolder: function (parentName, childFolders) {
        var folder, i, folderName, cls, contents = "",
            folderContents = "",
            placemarkContents = "",
            overlayContents = "",
            placemarks, sameName = false;
    ...
    // For Cable Color
    createMarkerColorString: function (markerColor) {
        if (markerColor !== undefined) {
            return 'clr-' + markerColor
        } else {
            return ""
        }
    },
    ...
    addSidebarList: function (placemarks) {
        var pm, placemark, marker, icon, coloor,  contents = "",
            innerContents = "",
            descrip = "",
            markerId, markerIcon, markerColor, id, itemList = [],
            itemListContents = [],
            folderHtml = '<span class="closed"></span>';
        for (pm = 0; pm < placemarks.length; pm++) {
            placemark = placemarks[pm];
            id = placemark.id;
            markerColor = placemark.style.color;
            color = this.createMarkerColorString(markerColor);
            if (placemark.marker) {
            ...
            if (this.showSidebarDescriptions) {
                ...
                if (placemark.marker) {
                    // For Nodes
                    innerContents = '<li class="placemark" data-tag="popSite" id="' + id + '">' + icon + placemark.name
                } else {
                    // For Paths
                    innerContents = '<li class="placemark" data-tag="cable" id="' + id + '">' + '<span class="' + color + '"></span>' + placemark.name
                }
            }
    ...
    addSidebarOverlayList: function (overlays) {
        var i, groundOverlay, overlay, contents = "",
            innerContents = "",
            descrip = "",
            itemList = [],
            itemListContents = [],
    ...
    getPlacemarkById: function (id) {
        ...
        if (aId.length > 0) {
            var docId = aId[0],
                ...
                newId = id.slice(0, j);
            if (docId) {
                var placemark, placemarkId, pm, placemarks = this.docSet.docs[docId].placemarks;
                for (pm = 0; pm < placemarks.length; pm++) {
                    placemark = placemarks[pm];
    ...
    getPlacemarksByName: function (folderId, placemarkName) {
        if (folderId) {
            var ids = this.getDocFolderIds(folderId),
                placemarks, pm, placemark, aPlacemarks = [],
                doc;
            if (ids !== undefined) {
    ...
    hilightShapesById: function (id) {
        var placemark, center, object = this.mapshapes[id];
        if (object === undefined) {
    ...
    changeShapeColors: function (placemarks, hilight) {
        var pm, placemark, points, point, lines, line, areas, area, circles, circle, markerLatLng, i, hilite = (hilight !== undefined) ? hilight : true,
            hiliteColor;
        if (hilite) {
                ...
                if (circles !== undefined) {
                    for (i = 0; i < circles.length; i++) {
                        ...
                        if (hilite) {
                            ...
                            circle.setOptions({
                                strokeColor: circle.origStrokeColor,
                                fillColor: circle.origFillColor
                            })
                        }
                    }
                } else {
                    if (lines !== undefined) {
                        for (i = 0; i < lines.length; i++) {
                            ...
                            if (hilite) {
                                ...
                                line.setOptions({
                                    strokeColor: line.origStrokeColor
                                })
                            }
                        }
                    } else {
                        if (areas !== undefined) {
                            for (i = 0; i < areas.length; i++) {
                                ...
                                if (hilite) {
                                    ...
                                    area.setOptions({
                                        strokeColor: area.origStrokeColor,
                                        fillColor: area.origFillColor
                                    })
                                }
                            }
                        }
...
function addOpactiy(id) {
    for (var ii = 0; ii < 140; ii++) {
        if (polyLines[ii].id !== id) {
            polyLines[ii].setOptions({
                strokeOpacity: '0.2'
            });
        } else {
            // polyLines[ii].setOptions({
